...
describe VestalVersions::Creation do
  ...
  context "a created version's changes" do
    ...
    it 'presents an array of versioned fields' do
      ...
      subject.versioned_columns.should include("first_name", "last_name")

    end

    it 'array of versioned fields includes fields restricted via :only' do
      User.prepare_versioned_options(:only => [:first_name])
      subject.versioned_columns.should_not include "last_name"
    ...
    it 'will respond to query if field is versioned' do
      User.prepare_versioned_options({})
